...
class Route
{
  ...
  public function addToDoc(ReflectionMethod $endpoint_ref): void
  {
    ...
    $controller_tags = [];
    $controller_tags_refs = $endpoint_ref->getDeclaringClass()->getAttributes(Tags::class);
    foreach ($controller_tags_refs as $controller_tags_ref) {
      $controller_tags = $this->mergeTags($controller_tags, $controller_tags_ref->newInstance()->tags);
    }
    $endpoint_tags = [];
    $endpoint_tags_refs = $endpoint_ref->getAttributes(Tags::class);
    foreach ($endpoint_tags_refs as $endpoint_tags_ref) {
      $endpoint_tags = $this->mergeTags($endpoint_tags, $endpoint_tags_ref->newInstance()->tags);
    }

    $tag_list = $this->mergeTags($this->tags, $this->mergeTags($controller_tags, $endpoint_tags));
...
//            $this->config = ['path' => $path, 'method' => $method, 'operation' => $operation];
  ...
  private function mergeTags($tags_1, $tags_2): array
  {
